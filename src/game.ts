...
export const upgrades: Upgrade[] = [
  { id: 'spaces', name: 'Stellplatz bauen', description: '+1 Parkplatz und mehr Kapazität', icon: 'P', category: 'Ausbau', maxLevel: 49, baseCost: 6, costMultiplier: 1.12 },
...
export const upgradeCost = (upgrade: Upgrade, level: number): number => {
  const rawCost = upgrade.baseCost * upgrade.costMultiplier ** level;
  return rawCost < 100 ? Math.round(rawCost) : Math.round(rawCost / 10) * 10;
};
...
export const incomePerSecond = (state: GameState): number =>
  incomePerMinute(state) * GAME_MINUTES_PER_TICK;

export const simulateTick = (state: GameState, random = Math.random): GameState => {
  const next: GameState = structuredClone(state);
  const minutes = GAME_MINUTES_PER_TICK;
  ...
  const earned = incomePerSecond(next);
  next.cash += earned;
...
export const setPrice = (state: GameState, price: number): GameState => {
  if (!Number.isFinite(price)) return state;
  const next = structuredClone(state);
  next.price = Math.round(Math.min(MAX_PRICE, Math.max(MIN_PRICE, price)) * 10) / 10;
  return next;
};
...
export const calculateOfflineProgress = (state: GameState, now: number): { state: GameState; earned: number; minutes: number } => {
  ...
  const offlineState = { ...state, occupied: averageOccupancy, activeIncident: null };
  const earned = incomePerSecond(offlineState) * elapsedMinutes * 60 * automationFactor;
  const next = structuredClone(state);
  next.cash += earned;
  next.lifetimeRevenue += earned;
